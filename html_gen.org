...
let (output_path, template_path, input_path) = if let Some(bd) = base_dir {
    let output_path = output_path.unwrap_or_else(|| bd.clone().push_str("/bin");
    let template_path = template_path.unwrap_or_else(|| bd.clone().push_str("/template");
    let input_path = input_path.unwrap_or_else(|| bd.clone().push_str("/src");
