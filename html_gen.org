...
#+begin_src rust :comments org :noweb yes
if let Some(value) = mapping.get(&caps[0]) {
   value.to_owned()
} else {
...
If the strategy is an ignore case, we simply leave the parameter as it was - (as our capture group automatically
drops the outermost braces, we must insert them ourselves.
