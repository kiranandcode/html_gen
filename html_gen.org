...
#+name: high level interface
#+begin_src rust :comments noweb
let (tname, mapping) = parser::parse_source_string("  \n#+template: simple\nbody: \\ Kiran is the best\\ \\¬¬\ntitle: We are the \\b best <champions></champions> ¬").unwrap();
println!("{:?}", generator::generate_output("<html>{{body}}\\{<\\html>".to_string(), mapping, &generator::GeneratorErrorStrategy::Base(generator::GeneratorErrorCoreStrategy::Fail)));
#+end_src
...
#+begin_src rust :noweb yes :comments org
let dir_name = Path::new(&input_file.file_name());
...
let input_text = {
   ...
   let file = File::open(input_file.path()).map_err(|e| CrawlError::InputFileError(format!("{:?}", e)))?;
...
let template_text = {
   let mut temp = String::new();
   let file = File::open(template_path).map_err(|e| CrawlError::TemplateNotFound(format!("{:?}", e)))?;
...
#+begin_src rust :noweb yes :comments org
let output_path = 
    output_directory.as_ref().join(&Path::new(&input_file.file_name()));
